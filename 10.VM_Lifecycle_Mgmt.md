...
Khi bắt đầu live migration, source libvirt ( SLibvirt ) sẽ liên lạc với destionation libvirt ( DLibvirt ) với các chi tiết của VM. DLibvirt sẽ chuyển thông tin đến QEMU với các option liên quan để bật chế độ live migration. QEMU sẽ bắt đầu quá trình live migration bằng cách bắt đầu với VM ở chế độ pause và bắt đầu lắng nghe trên TCP port để lấy VM data. Một DLibvirt sẽ nhận thông báo từ SLibvirt với chi tiết của QEMU. Đồng thời QEMU tại source sẵn sàng di chuyển VM và connect với destination TCP port
