...
- **dump-flows** : In ra tất cả flow entries trong switch
- **dump-flows** *switch* *flow* : In ra flow *flow*
...
-**action** : 
   ...
   - **output**:*port* : Output packet đến port *port* của OpenFlow
   - **group**:*group_id* : packt đi ra đến *group_id* của OpenFlow
   - **all** : Đi ra tất cả các cổng của physical switch trừ port được nhận
   - **ip_port** : Đi ra từ port và nó nhận được packet
   - **drop** : Vứt bỏ gói tin và không có hành động gì được diễn ra 

Đọc thêm : https://manpages.debian.org/testing/openvswitch-common/ovs-ofctl.8.en.html
